...
printk("1,ADCTSC=0x%lx\n",tmp);
data = readl(peter_base_addr+S3C2410_ADCDAT0);
printk("before everything, ADCDAT0=0x%x",data); 
tmp = readl(peter_base_addr+S3C2410_ADCCON) |S3C2410_ADCCON_ENABLE_START;
printk("ADCCON=0x%lx\n",tmp);
...
//only the lower 10 bits is meaningful
 printk("hey i am printk, the output data=%d\n", data);
...
tmp&=0xfff8;//clear the last three bits
 
writel(tmp , peter_base_addr+S3C2410_ADCTSC);
printk("1,write, ADCTSC=0x%lx\n",tmp);


 //S3C2410_ADCCON----set chanel and prescaler
 tmp = readl(peter_base_addr+S3C2410_ADCCON);
 tmp |= S3C2410_ADCCON_PRSCEN | S3C2410_ADCCON_PRSCVL(adcdev.prescale);
// | ADC_INPUT(adcdev.channel); let channel be 0
 writel( tmp, peter_base_addr+S3C2410_ADCCON);
 printk("3,ADCCON=0x%lx",tmp);
...
unsigned long tmp2;
tmp2=readl(S3C2410_CLKCON);
printk("open, CLKCON=0x%lx",tmp2);
 printk("ad------------------- opened!\n");
 return 0;
}
